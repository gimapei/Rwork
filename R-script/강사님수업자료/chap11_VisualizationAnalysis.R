...
num <- equal.count(1:150, number=3, overlap=0)
num
...
university <- read.csv("C:/Rwork/Part-III/university.csv",header=T, encoding = 'utf-8')
university # # 학교명","LAT","LON"
names(university) <- c('NAME','LAT','LON')
...
# (2)레이어2 : 지도위에 포인트
ggmap(kor)+geom_point(data=university, aes(x=LON, y=LAT,color=factor(NAME)),size=3)
kor.map <- ggmap(kor)+geom_point(data=university, aes(x=LON, y=LAT,color=factor(NAME)),size=3)

# (3)레이어3 : 지도위에 텍스트 추가
kor.map + geom_text(data=university, aes(x=LON+0.01, y=LAT+0.01,label=NAME),size=5)
...
tot_pop <- read.csv('population201506.csv', header = T)
head(tot_pop)
total <- tot_pop$총인구수
lon <- tot_pop$LON
lat <- tot_pop$LON
...
df <- data.frame(total, lon, lat, region)
df
...
# 레이어2 : 포인트 추가
map2 + geom_point(aes(x=lon,y=lat,colour=total,size=total),data=df)
map3 <- map2 + geom_point(aes(x=lon,y=lat,colour=total,size=total),data=df)
