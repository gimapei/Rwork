...
prop.table(x1) # 빈도수 비율 계산
y1 <-  prop.table(x1)
round(y1*100, 2) #백분율 적용(소수점 2자리)
...
mean(data$cost, na.rm = T) # NA -  8.783521
round(mean(data$cost, na.rm = T), 3) # 8.784
data$cost
...
# 연속형 -> 범주화(리코딩) : 1,2,3
names(data)
data$cost2[data$cost >=1 & data$cost <=3] <-1  # 1 ~ 3
data$cost2[data$cost >=4 & data$cost <=6] <-2 # 4 ~ 6
data$cost2[data$cost >=7] <-3 # 7이상 
...
head(data[c('resident', 'resident2')])
...
#<연습문제2> descriptive.csv 데이터 셋을 대상으로 다음 조건에 맞게 빈도분석 및 기술통계량 분석을 수행하시오.
data
#[조건 1] 명목척도 변수인 학교유형(type), 합격여부(pass) 변수에 대해 빈도분석을 수행하고 결과를 막대그래프나 파이차트로 그리시오.
type <- data$type 
table(type)
barplot(table(type))
pie(table(type))
...
hist(age) 
# [해석] 왜도가 0보다 크기 때문에 오른쪽 방향으로 
# 비대칭 꼬리가 치우치고, 첨도는 3보다 작기 때문에 표준정규분포보다 
# 완만한 형태를 갖는다.
